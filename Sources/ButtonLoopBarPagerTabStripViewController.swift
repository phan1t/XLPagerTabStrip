...
//  MatchQ
//
//  Created by Son Chanthem on 1/25/19.
//  Copyright © 2019 BENITEN CO., LTD. All rights reserved.
//
...
open class ButtonLoopBarPagerTabStripViewController: ButtonBarPagerTabStripViewController {
  ...
  override open var cachedCellWidths: [CGFloat]? {
  ...
  private func shiftButtonsIndex() {
    ...
    if mostLeftVisibleIndex == 0 {
      shiftIndex = -(visibleIndexs.count) / 2
    ...
    let buttonBarShiftWidth: CGFloat = shiftIndex == (visibleIndexs.count) / 2 ? -cachedCellWidths![(visibleIndexs.count) / 2] : cachedCellWidths![(visibleIndexs.count) / 2]
    ...
    UIView.performWithoutAnimation {
      ...
      buttonBarView.layer.removeAllAnimations()
    }
